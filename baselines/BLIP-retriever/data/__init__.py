...
from data.webqa_dataset import webqa_dataset, webqa_eval_dataset
from transform.randaugment import RandomAugment

def create_dataset(dataset, config, min_scale=0.5, args=None):
    ...
    elif dataset=='retrieval_webqa':
        train_dataset = webqa_dataset(transform_train, config['image_root'], config['train_val_root'], split='train', prompt= config['prompt'])
        val_dataset  = webqa_eval_dataset(args, transform_test, config['image_root'], config['train_val_root'], split='val', prompt= config['prompt'])
        # test_dataset   = webqa_test_dataset(transform_test, config['image_root'], config['train_val_root'], split='test', prompt= config['prompt'])
        return train_dataset, val_dataset #, test_dataset
