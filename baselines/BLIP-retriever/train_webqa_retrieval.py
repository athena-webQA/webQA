...
@torch.no_grad()
def evaluation(model, text_loader, image_loader,device, config):
    # test
    model.eval() 
    ...
    score_matrix_i2t = torch.full((len(image_loader),len(text_loader)),-100.0).to(device)
    ...
    score_matrix_t2i = torch.full((len(text_loader),len(image_loader)),-100.0).to(device)
...
def main(args, config):
    ...
    for epoch in range(0, config['max_epoch']):    
        ...
        score_val_i2t, score_val_t2i, = evaluation(model_without_ddp, val_question_loader, val_image_loader, device, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='pretrained_retrieved_results.json')        
